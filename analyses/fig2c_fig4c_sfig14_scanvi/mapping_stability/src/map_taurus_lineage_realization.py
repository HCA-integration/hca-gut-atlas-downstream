#!/usr/bin/env python3
"""Map fixed TAURUS lineage query through one saved SCANVI realization.

Saves hard labels + soft probabilities. Never discards outputs.
"""
...
LOGGER = logging.getLogger("map_taurus_lineage")


def _load_lineage_query(lineage: str, max_cells: int | None, seed: int) -> anndata.AnnData:
    ann = pd.read_parquet(ANNOT_PARQUET)
    sub = ann[ann["assigned_lineage"].astype(str) == lineage].copy()
    ...
    ok = pos >= 0
    if ok.mean() < 0.99:
        raise RuntimeError(f"{lineage} barcode match too low: {ok.mean():.3f}")
    pos = pos[ok]
    sub = sub.loc[ok].copy()
...
def map_one(
    model_dir: Path, out_dir: Path, lineage: str, max_cells: int | None, seed: int
) -> Path:
    ...
    train_lineage = train_man.get("lineage", lineage)
    if train_lineage != lineage:
        raise ValueError(f"model lineage {train_lineage} != requested {lineage}")

    LOGGER.info("Loading query %s cells", lineage)
    query_raw = _load_lineage_query(lineage, max_cells, seed)
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"--model-dir must be under {models_root}, got {model_dir}") from exc
    ...
    map_one(model_dir, out_dir, args.lineage, args.max_cells, args.seed)
    return 0
