#!/usr/bin/env python3
"""Primary myeloid screening metrics (same definitions as stroma)."""
...
LOGGER = logging.getLogger("myeloid_metrics")
TAUS = (0.70, 0.80, 0.90, 1.00)
PSEUDO = 0.5
SHARED = MANIFESTS / "myeloid_shared_studies_exact_name.csv"
...
def _pangi_paths() -> dict[str, list[str]]:
    hier = pd.read_csv(MANIFESTS / "pangi_myeloid_taxonomy_edges.csv")
    paths = {}
    for _, row in hier.iterrows():
        leaf = str(row["level_3_annot"])
        path = ["Myeloid", str(row["level_1_annot"]), str(row["level_2_annot"]), leaf]
        out = []
        for p in path:
...
def _stable_depth_for_cell(leaves, paths, tau):
    cell_paths = [paths.get(leaf, ["Myeloid", leaf]) for leaf in leaves]
    n = len(cell_paths)
    max_len = max(len(p) for p in cell_paths)
    deepest_node = "Myeloid"
    deepest_idx = 0
...
def _load_preds(atlas: str, kind: str) -> pd.DataFrame:
    base = PREDICTIONS / atlas / "myeloid"
    frames = []
    for d in sorted(base.glob("*/seed*")):
        ...
        frames.append(df)
    if not frames:
        raise FileNotFoundError(f"No myeloid predictions for {atlas} {kind}")
...
def size_adjust(disp: pd.DataFrame) -> dict:
    impact = pd.read_csv(MANIFESTS / "myeloid_study_omission_impact.csv")
...
def main() -> int:
    ...
    for atlas in ("HGCA", "PanGI"):
        ...
        depth.to_parquet(TABLES / f"myeloid_stable_naming_depth_cells_{atlas}.parquet", index=False)
    ...
    depths.to_parquet(TABLES / "myeloid_stable_naming_depth_cells.parquet", index=False)
    summary = (
    ...
    summary.to_csv(TABLES / "myeloid_stable_naming_depth_summary.csv", index=False)
    pd.DataFrame(seed_rows).to_csv(TABLES / "myeloid_seed_leaf_agreement_summary.csv", index=False)

    all_d = pd.concat(disp_frames, ignore_index=True)
    all_d.to_parquet(TABLES / "myeloid_sample_aitchison_displacement.parquet", index=False)
    dsum = (
        all_d.groupby(["atlas", "omitted_study"])
        .agg(median_dist=("aitchison_to_full_seed0", "median"), mean_dist=("aitchison_to_full_seed0", "mean"))
        .reset_index()
    )
    dsum.to_csv(TABLES / "myeloid_sample_aitchison_displacement_summary.csv", index=False)
    overall = all_d.groupby("atlas")["aitchison_to_full_seed0"].agg(["median", "mean"]).reset_index()
    overall.to_csv(TABLES / "myeloid_sample_aitchison_displacement_overall.csv", index=False)
    size = size_adjust(all_d)
    (TABLES / "myeloid_size_adjusted_displacement.json").write_text(json.dumps(size, indent=2) + "\n")
    ...
    (TABLES / "myeloid_screen_metrics_manifest.json").write_text(json.dumps(man, indent=2, default=str) + "\n")
