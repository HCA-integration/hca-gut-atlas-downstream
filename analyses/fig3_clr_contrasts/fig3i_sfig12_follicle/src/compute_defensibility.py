...
Reads ../data/clr_long.csv (and lineage map) and
...
def load_clr(path: Path | None = None) -> pd.DataFrame:
    d = pd.read_csv(path or REPO_ROOT / "data" / "composition" / "clr_long.csv")
...
def main() -> None:
    ...
    parser.add_argument(
        "--clr-long",
        type=Path,
        default=REPO_ROOT / "data" / "composition" / "clr_long.csv",
    )
