...
| Supplementary Figure 3 | Integration benchmark via [scAtlasTb](https://scatlastb.readthedocs.io) | [`sfig3_scAtlasTb`](sfig3_scAtlasTb/) | Benchmark subset built by `src/integration_prep_atlasTb.ipynb`; config `src/hgca_integration_benchmark.yaml`. Cluster + scAtlasTb environments |
...
| Figure 3b published effect sizes | One-way R² / η² table that matches the submitted heatmap numbers | [`data/fig3/`](../data/fig3/) | Frozen `celltype_metadata_sensitivity_top2.csv`. Not the segment-stratified ω² audit |
...
| Figure 3g, 3j | Cell types in Xenium spatial context | [`fig3_clr_contrasts/fig3gj_xenium`](fig3_clr_contrasts/fig3gj_xenium/) | `data/xenium_controls_only.h5ad` and `data/hgca_cell_type_reference.tsv`; morphology panels need the raw Xenium bundles (`XENIUM_ROOT`) |
...
| Figure 4b, 4e | TAURUS scANVI mapping and per-lineage label transfer; Milo enrichment in inflammation | [`fig4be_mapping_milo`](fig4be_mapping_milo/) | TAURUS h5ad and the HGCA lineage scANVI models; GPU |
| Figure 4f, 4h–i; Supplementary Figure 16b | Sample representations with [patpy](https://github.com/lueckenlab/patpy) and sampleCLR | [`fig4fhi_sfig16b_patpy_sampleCLR`](fig4fhi_sfig16b_patpy_sampleCLR/) | TAURUS h5ad mapped in `fig4be_mapping_milo`. sampleCLR is unpublished and is released with the manuscript |
